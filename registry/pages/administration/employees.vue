<template>
  <VCard v-if="loading == true && nright == false" max-width="400" class="mx-auto" elevation="0" loading
    title="Идет загрузка...">
  ...
  <v-card v-if="nright" max-width="400" class="mx-auto" elevation="0" loading>
    <v-card-text class="text-h6">У вас нет доступа к этой странице, сейчас вы будете перенаправленны на панель
      управления.</v-card-text>
    <img src="../../public/cat-laptop-rights.jpg" alt="cat with laptop" class="w-50 d-inline mx-auto">
  </v-card>
  <VRow class="ma-1 flex-nowrap">
    <Table v-show="loading === false && nright == false && data.length > 0"
      @cheked="checkEmpl = $event, disabledFunc(), loadEmplData()" @person="checkEmpl = $event, loadEmplData()"
      :info="filteredData.length ? filteredData : data" :checkbox-show="show" :rights="empRights" :page="page"
      :headers="th" :actions="tableActions"></Table>
    ...
    <v-expand-x-transition>
      <VCard v-show="drawer" class="mb-auto mx-1" :location="filtLoc" :position="filtPos">
        <VForm v-model="form" @keydown.enter="btnDis() ? btnDis() : (search(), page = 1)"
          @keyup.delete="(e) => { if (e.key == 'Delete') { fio = '', phone = '', email = '', params = [], value = [], stopAutoReq() } }">
          <VCol>
            ...
            <VTextField v-model="fio" v-if="empRights.empProfRights.includes('r')" clearable
              hint="Введите минимум 2 символа" ref="fioF"
              @click:clear="() => { filterItems('', th[0].key), fio = '', stopAutoReq() }"
              @update:focused="lastField = fioF, searchField = false" :label="th[0].title" variant="underlined"
              color="secondary" @update:model-value="() => { filterItems(fio, th[0].key), autoReq(fio) }" />
            <VTextField v-model="phone" v-if="empRights.empContRights.includes('r')" v-maska:[phoneOptions] clearable
              hint="Введите минимум 6 цифр" ref="phoneF"
              @click:clear="() => { filterItems('', th[1].key), phone = '', stopAutoReq() }"
              @update:focused="lastField = phoneF, searchField = false" :label="th[1].title" variant="underlined"
              color="secondary" @input="autoReq(phone.replace(/[+() --]/g, '').trim())"
              @update:model-value="() => { filterItems(phone.replace(/[+() --]/g, '').trim(), th[1].key) }" />
            <VTextField v-model="email" v-if="empRights.empContRights.includes('r')" clearable
              hint="Введите минимум 3 символа" ref="emailF"
              @click:clear="() => { filterItems('', th[2].key), email = '', stopAutoReq() }"
              @update:focused="lastField = emailF, searchField = false" :label="th[2].title" variant="underlined"
              color="secondary" @input="autoReq(email)" @update:model-value="() => { filterItems(email, th[2].key) }" />
            <VTextField v-model="itemPerPage" label="Количество элементов на странице" min="5" max="100" step="5"
              variant="underlined" color="secondary" type="number" @input="itemPerPage > 5 ? true : itemPerPage = 5">
            </VTextField>
  ...
  <v-bottom-navigation bg-color="tertiary" elevation="0" grow>
    <v-pagination class="w-100" v-if="data.length" v-show="!drawer || pagVis" rounded="pill" :length="data.length"
      v-model="page" :total-visible="7" :size="pagSize"></v-pagination>
  </v-bottom-navigation>
...
const checkRole = async () => {
  ...
  let r = (await recStore.getOrCreate(EmployeeRecord, k!)).MData?.roles;
  //Запрос роли сотрудника и проверка прав доступа
  let rec = await recStore.fetch(RolesRecord, k!);
  allRoles.value = Object.keys(rec.MData.roles.roles);
  role.value = rec.MData.roles.roles[r!];
  // role.value = {
  //   "dbEmployee": "cruds",
