<template>
  <div v-if="visibility" style="height: 10%" class="d-flex flex-column">
                        ...
                        <v-list-item v-for="val in accessibleColItems">
                          <template v-slot:prepend="{ isActive }">
                            <v-list-item-action start>
                              <v-checkbox-btn
                                :model-value="columns.includes(val.key)"
                                @update:modelValue="
    ...
    <v-row
      class="text-center pt-5 w-100 mb-1 pr-0"
      justify="start"
      ...
      >
        <v-row justify="center">
          ...
          <v-select
            ...
            label="На странице"
            :items="[10, 12, 25, 50, 100]"
            variant="solo"
            @update:model-value="
              itemsPerPage = parseInt(<string>(<unknown>$event), 10)
            "
          ></v-select>
        </v-row>
      </v-col>
...
let selectedValTitleMap = new Map(
  props.selected?.map((obj) => [obj.value, obj.title || ""])
);
let refDt = ref(null);
let refPag = ref();
const classMap = {
  start: "d-flex justify-start",
  center: "d-flex justify-center mr-5",
...
props.tableDescr.headers.forEach((item) => {
  if (!chkRights(null, item.traits)) notAccessibleCols.value.push(item.key);
  else {
...
const _headers = computed(() => {
  const res: any[] = [
    { key: "actions", align: "start", width: "10", sortable: false, title: "" },
  ];

  props.columns.forEach((item) => {
    let headerItem = accessibleColItems.value.find((el) => el.key == item);
    if (headerItem) res.push(headerItem);
  });
...
const getActionsMenu = (item: any) => {
  ...
  menu.forEach((item) => {
    if (chkRights(null, item.traits)) res.push(item);
  });
  return res;
};
...
const clearChangeTimeout = () => {
  if (colChangeTimeout) clearTimeout(colChangeTimeout);
  colChangeTimeout = null;
};

const toggleSelectColumn = (e, colName: string) => {
  let inx = props.columns.indexOf(colName);
  if (inx == -1) props.columns.push(colName);
  else props.columns.splice(inx, 1);

  clearChangeTimeout();

  colChangeTimeout = setTimeout(
    () => emit("onColumnsChangedDelayed", props.columns),
    3000
  );
};
...
watch(
  ...
  () => {
    mapRows = new Map(
      props.rows.map((obj) => [
        obj.id,
        props.titleColName ? obj[props.titleColName] : "",
      ])
    );
  },
  { deep: true }
