...
export class RelationApiSection {
  ...
  async addChild(parentId: string, parentRecCode: number, childId: string, childRecCode: number, relType: number) {
    return await this._apiClient.send<any, boolean>(`${_apiPath}/AddChild`, { parentId, parentRecCode, childId, childRecCode, relType }, true);
  }
