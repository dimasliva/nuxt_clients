    ...
    <v-card-text>
      <v-form v-model="form" @submit.prevent="actionEmpl">
        <v-container>
          <v-row class="pa-6">
            <v-col cols="12" sm="6">
              <v-text-field label="Имя" clearable v-model="empName" v-maska:[fioOptions] :autofocus="updProf"
                :disabled="!updProf" required maxlength="128" variant="underlined" placeholder="Иван" density="compact"
                :rules="[(v: string) => !!v || $t('required')]"
                @input="empName = translit(empName).charAt(0).toUpperCase() + translit(empName).slice(1).toLowerCase()"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <v-text-field label="Фамилия" clearable v-model="empSurname" v-maska:[fioOptions] :disabled="!updProf"
                required maxlength="128" variant="underlined" placeholder="Иванов" density="compact"
                :rules="[(v: string) => !!v || $t('required')]"
                @input="empSurname = translit(empSurname).charAt(0).toUpperCase() + translit(empSurname).slice(1).toLowerCase()"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <v-text-field label="Отчество" clearable v-model="empPatronymic" v-maska:[fioOptions] :disabled="!updProf"
                maxlength="128" variant="underlined" placeholder="Иванович" density="compact"
                @input="empPatronymic = translit(empPatronymic).charAt(0).toUpperCase() + translit(empPatronymic).slice(1).toLowerCase()"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <v-text-field v-model="empEmail" label="Email" :disabled="!updCont" placeholder="ivanov@mail.com" required
                maxlength="64" variant="underlined" density="compact"
                :rules="[(v: string) => !!v || $t('required'), (v: string) => (/.+@.+\..+/.test(v)) || $t('vemail')]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <v-text-field clearable v-model="empPhone" :disabled="!updCont" placeholder="+7(999) 999-99-99"
                v-maska:[phoneOptions] required maxlength="24" label="Телефон" variant="underlined" density="compact"
                :rules="[(v: string) => !!v || $t('required')]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6" v-if="crtEmpl && adding">
              <v-text-field label="Логин" :disabled="!addAccount" clearable v-model="empLogin"
                @click:control="empLogin = empEmail" :required="addAccount" maxlength="128" variant="underlined"
                placeholder="Ivan001" density="compact"
                :rules="addAccount ? [(v: string) => !!v || $t('required')] : undefined"></v-text-field>
            </v-col>
            <v-col cols="12" sm="6">
              <VueDatePicker v-model="empBirthdate" :enable-time-picker="false" model-type="yyyy-MM-dd" locale="ru"
                auto-apply teleport-center>
                <template #trigger>
                  <v-text-field v-model="empBirthdate" density="compact" type="date" :disabled="!updProf"
                    variant="underlined" :rules="[(v: string) => !!v || $t('required')]" required clearable>
            ...
            <v-col cols="12" sm="6">
              <v-select v-model="empGender" density="compact" label="Пол" :disabled="!updProf"
                :items="[{ gender: 'м', val: 'm' }, { gender: 'ж', val: 'f' }]" item-title="gender" item-value="val"
                variant="underlined"></v-select>
            </v-col>
            <v-col v-if="specEmpl" cols="12" sm="6">
              <v-select v-model="empRole" density="compact" label="Роль" :disabled="!updProf" :items="roles"
                variant="underlined" multiple></v-select>
            </v-col>
          </v-row>
          <v-checkbox class="ml-4" v-if="adding" label="создать аккаунт" v-model="addAccount"
