<template>
    <v-row>
      <v-col>
          <v-table density="compact" class="rounded mx-2 elevation-1" :height="theight" fixed-header hover>
            <thead>
              <tr>
                ...
                <th class="ma-0 pa-0 bg-primary" v-if="delRights"></th>
            ...
            <tbody>
              <tr v-for="item in info[page! - 1]" :key="item.id" @mouseenter="$emit('person', item)" @dblclick="props.actions[0].action">
                ...
                <td class="text-center pa-0">
                  {{ item.surname +" "+ item.name +" "+ (item.patronymic? item.patronymic: '')  }}
                </td>
                <td class="text-center pa-0">
                  {{ item.mainPhone }}
                </td>
